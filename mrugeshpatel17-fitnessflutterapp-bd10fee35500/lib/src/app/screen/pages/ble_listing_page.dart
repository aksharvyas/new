...
class BLEListingPageState extends State<BLEListingPage> {
  ...
  initialize(BluetoothDevice device, state) async {
    ...
    Future.delayed(Duration(seconds: 2), () async {
      ...
      uploadCsvOnAPI(file);
        ...
        print(" " + (EndTime.difference(startTime).toString()));
        // if (services != null) {
  ...
  void uploadCsvOnAPI(File filename) async {
    ...
    if (response.statusCode == 200) {
      ...
      showToast("Successfully upload csv file"+ response.statusCode.toString());
    } else {
      print("Upload Failed");
      showToast("Upload Failed"+response.statusCode.toString());
    }
    response.stream
